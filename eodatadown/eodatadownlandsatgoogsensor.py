...
class EDDLandsatGooglePlugins(Base):
    __tablename__ = "EDDLandsatGooglePlugins"
...
class EODataDownLandsatGoogSensor (EODataDownSensor):
                ...
                if len(query_rtn) == 0:
                    ...
                    db_records.append(
                        EDDLandsatGoogle(PID=n_max_pid, Scene_ID=row.scene_id, Product_ID=row.product_id,
                                         ...
                                         Date_Acquired=row.date_acquired, #datetime.datetime.strptime(row.date_acquired, "%Y-%m-%d").date(),
    ...
    def get_scnlist_usr_analysis(self):
        ...
        if self.calc_scn_usr_analysis():
            ...
            for scn in query_result:
                scn_plgin_db_objs = ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.Scene_PID == scn.PID).all()
    ...
    def has_scn_usr_analysis(self, unq_id):
        ...
        if self.calc_scn_usr_analysis():
            ...
            scn_plgin_db_objs = ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.Scene_PID == unq_id).all()
            ses.close()
            logger.debug("Closed the database session.")
            if (scn_plgin_db_objs is None) or (not scn_plgin_db_objs):
    ...
    def run_usr_analysis(self, unq_id):
        if self.calc_scn_usr_analysis():
            for plugin_info in self.analysis_plugins:
                ...
                plgin_db_obj = ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.Scene_PID == unq_id,
                                                                         EDDLandsatGooglePlugins.PlugInName == plugin_key).one_or_none()
                plgin_db_objs = ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.Scene_PID == unq_id).all()
                ses.close()
                logger.debug("Closed the database session.")
                ...
                if not plugin_completed:
                    ...
                    if exists_in_db:
                        ...
                        ses = session_sqlalc()
                        logger.debug("Perform query to find scene in plugin DB.")
                        plgin_db_obj = ses.query(EDDLandsatGooglePlugins).filter(
                            EDDLandsatGooglePlugins.Scene_PID == unq_id,
                            EDDLandsatGooglePlugins.PlugInName == plugin_key).one_or_none()
                        ...
                        plgin_db_obj = EDDLandsatGooglePlugins(Scene_PID=scn_db_obj.PID, PlugInName=plugin_key,
            ...
            if len(plgin_lst) > 0:
                ...
                if scn_pid is None:
                    ...
                    for plgin_key in plgin_lst:
                        ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.PlugInName == plgin_key).delete(synchronize_session=False)
                        ses.commit()
                else:
                    logger.debug("Scene PID {} has been provided so resetting.".format(scn_pid))
                    if reset_all_plgins:
                        ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.Scene_PID == scn_pid).delete(synchronize_session=False)
                    else:
                        scn_plgin_db_objs = ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.Scene_PID == scn_pid).all()
    ...
    def export_db_to_json(self, out_json_file):
        ...
        if self.calc_scn_usr_analysis():
            ...
            for plgin_key in plugin_keys:
                query_result = ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.PlugInName == plgin_key).all()
            ...
            if 'plgin_db' in db_data:
                ...
                for plgin_key in plgin_rows:
                    for scn_pid in plgin_rows[plgin_key]:
                        db_plgin_records.append(EDDLandsatGooglePlugins(Scene_PID=plgin_rows[plgin_key][scn_pid]['Scene_PID'],
    ...
    def get_sensor_summary_info(self):
        ...
        if self.calc_scn_usr_analysis():
            ...
            for plgin_key in plgin_lst:
                ...
                scns = ses.query(EDDLandsatGooglePlugins).filter(EDDLandsatGooglePlugins.PlugInName == plgin_key).all()
                n_err_scns = 0
                n_complete_scns = 0
                n_success_scns = 0
                plugin_times = []
