...
def _process_to_ard(params):
    ...
    try:
        ...
        input_xml = edd_utils.findFile(scn_path, file_identifier+"*metadata.xml")
...
class EODataDownPlanetScopeSensor (EODataDownSensor):
    ...
    def __init__(self, db_info_obj):
        ...
        self.sensor_name = "PlanetScope"
        self.db_tab_name = "EDDPlanetScope"
    ...
    def get_scnlist_all(self):
        ...
        query_result = ses.query(EDDPlanetScope).all()
        scns = []
    ...
    def has_scn_download(self, unq_id):
        ...
        ses = session_sqlalc()
        logger.debug("Perform query to find scenes which need downloading.")
        query_result = ses.query(EDDPlanetScope).filter(EDDPlanetScope.PID == unq_id).one()
        ses.close()
        logger.debug("Closed the database session.")
    ...
    def scns2ard_all_avail(self, n_cores):
        ...
        query_result = ses.query(EDDPlanetScope).filter(EDDPlanetScope.Downloaded == True,
                                                        EDDPlanetScope.ARDProduct == False,
                                                        EDDPlanetScope.Invalid == False).all()
        ...
        if query_result is not None:
            ...
            for record in query_result:
                ...
                ard_params.append([record.Scene_ID, record.File_Identifier, self.db_info_obj, record.Download_Path, work_ard_scn_path, tmp_ard_scn_path, final_ard_scn_path, self.ardProjDefined, proj_wkt_file, self.projabbv])
        else:
    ...
    def get_scnlist_datacube(self, loaded=False):
        ...
        logger.debug("Perform query to find scenes which need converting to ARD.")
        query_result = ses.query(EDDPlanetScope).filter(EDDPlanetScope.ARDProduct == True,
                                                        EDDPlanetScope.DCLoaded == loaded).all()
        scns2dcload = []
        if query_result is not None:
            for record in query_result:
    ...
    def reset_scn(self, unq_id, reset_download=False):
        ...
        logger.debug("Perform query to find scene.")
        scn_record = ses.query(EDDPlanetScope).filter(EDDPlanetScope.PID == unq_id).one_or_none()
