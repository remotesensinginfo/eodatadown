...
class EDDSentinel2GooglePlugins(Base):
    __tablename__ = "EDDSentinel2GooglePlugins"
...
class EODataDownSentinel2GoogSensor (EODataDownSensor):
    ...
    def has_scn_usr_analysis(self, unq_id):
        ...
        if self.calc_scn_usr_analysis():
            ...
            scn_plgin_db_objs = ses.query(EDDSentinel2GooglePlugins).filter(EDDSentinel2GooglePlugins.Scene_PID == unq_id).all()
            ses.close()
            logger.debug("Closed the database session.")
            if (scn_plgin_db_objs is None) or (not scn_plgin_db_objs):
    ...
    def run_usr_analysis(self, unq_id):
        if self.calc_scn_usr_analysis():
            for plugin_info in self.analysis_plugins:
                ...
                plgin_db_obj = ses.query(EDDSentinel2GooglePlugins).filter(EDDSentinel2GooglePlugins.Scene_PID == unq_id, EDDSentinel2GooglePlugins.PlugInName == plugin_key).one_or_none()
                plgin_db_objs = ses.query(EDDSentinel2GooglePlugins).filter(EDDSentinel2GooglePlugins.Scene_PID == unq_id).all()
                ses.close()
                logger.debug("Closed the database session.")
                ...
                if not plugin_completed:
                    ...
                    if exists_in_db:
                        ...
                        plgin_db_obj = EDDSentinel2GooglePlugins(Scene_PID=scn_db_obj.PID, PlugInName=plugin_key,
            ...
            if len(plgin_lst) > 0:
                ...
                if scn_pid is None:
                    ...
                    for plgin_key in plgin_lst:
                        ses.query(EDDSentinel2GooglePlugins).filter(EDDSentinel2GooglePlugins.PlugInName == plgin_key).delete(synchronize_session=False)
                        ses.commit()
                else:
                    logger.debug("Scene PID {} has been provided so resetting.".format(scn_pid))
                    if reset_all_plgins:
                        ses.query(EDDSentinel2GooglePlugins).filter(EDDSentinel2GooglePlugins.Scene_PID == scn_pid).delete(synchronize_session=False)
                    else:
                        scn_plgin_db_objs = ses.query(EDDSentinel2GooglePlugins).filter(EDDSentinel2GooglePlugins.Scene_PID == scn_pid).all()
    ...
    def query_scn_records_bbox(self, lat_north, lat_south, lon_east, lon_west):
        ...
        query_result = ses.query(EDDSentinel2Google).filter(EDDSentinel2Google.Invalid == False,
                                                            EDDSentinel2Google.ARDProduct == True).filter(
                                                            (lon_east > EDDSentinel2Google.West_Lon),
                                                            (EDDSentinel2Google.East_Lon > lon_west),
                                                            (lat_north > EDDSentinel2Google.South_Lat),
                                                            (EDDSentinel2Google.North_Lat > lat_south)).order_by(
                                                            EDDSentinel2Google.Sensing_Time.desc()).all()
        ses.close()
        logger.debug("Closed the database session.")
    ...
    def export_db_to_json(self, out_json_file):
        ...
        if self.calc_scn_usr_analysis():
            ...
            for plgin_key in plugin_keys:
                query_result = ses.query(EDDSentinel2GooglePlugins).filter(EDDSentinel2GooglePlugins.PlugInName == plgin_key).all()
                ...
                if 'plgin_db' in db_data:
                    ...
                    for plgin_key in plgin_rows:
                        for scn_pid in plgin_rows[plgin_key]:
                            db_plgin_records.append(EDDSentinel2GooglePlugins(Scene_PID=plgin_rows[plgin_key][scn_pid]['Scene_PID'],
    ...
    def get_sensor_summary_info(self):
        ...
        vld_scn_count = ses.query(EDDSentinel2Google).filter(EDDSentinel2Google.Invalid == False).count()
        invld_scn_count = ses.query(EDDSentinel2Google).filter(EDDSentinel2Google.Invalid == True).count()
        dwn_scn_count = ses.query(EDDSentinel2Google).filter(EDDSentinel2Google.Downloaded == True).count()
        ard_scn_count = ses.query(EDDSentinel2Google).filter(EDDSentinel2Google.ARDProduct == True).count()
        dcload_scn_count = ses.query(EDDSentinel2Google).filter(EDDSentinel2Google.DCLoaded == True).count()
        arch_scn_count = ses.query(EDDSentinel2Google).filter(EDDSentinel2Google.Archived == True).count()
        ...
        file_sizes = ses.query(EDDSentinel2Google.Total_Size).filter(EDDSentinel2Google.Invalid == False).all()
        ...
        download_times = []
        ard_process_times = []
        scns = ses.query(EDDSentinel2Google).filter(EDDSentinel2Google.Downloaded == True)
        for scn in scns:
        ...
        if self.calc_scn_usr_analysis():
            ...
            for plgin_key in plgin_lst:
                ...
                scns = ses.query(EDDSentinel2GooglePlugins).filter(EDDSentinel2GooglePlugins.PlugInName == plgin_key).all()
                n_err_scns = 0
                n_complete_scns = 0
                n_success_scns = 0
                plugin_times = []
